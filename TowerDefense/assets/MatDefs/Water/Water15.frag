...
#ifdef ENABLEDMAP
  uniform sampler2D m_EnabledMap;
#endif
...
*/
    ...
    if(position.x+0.5  > m_Size/2 || position.z+0.5  > m_Size/2|| position.x-0.5 < -m_Size/2 || position.z-0.5 < -m_Size/2)
    {
        return vec4(color.r,color.g,color.b,1);
    }

    #ifdef ENABLEDMAP
        vec2 temppos = vec2(position.x/m_Size+0.5,1-(position.z/m_Size+0.5));

        vec4 enablecolor = texture2D(m_EnabledMap, temppos.xy);
    ...
    #ifdef FOGMAP
        vec2 pos = vec2(position.x/m_Size+0.5,1-(position.z/m_Size+0.5));

        vec4 fogcolor = texture2D(m_FogMap, pos.xy);

        if(fogcolor.a != 0)
        {
            color = mix( color, fogcolor.xyz, fogcolor.a );
        }
       // if(fogcolor.a != 0)
        //{
         //   color= vec3(fogcolor.r,fogcolor.g,fogcolor.b);
        //    color2= color;
       //     normal = vec3(0.0);
      //  }
    #endif
