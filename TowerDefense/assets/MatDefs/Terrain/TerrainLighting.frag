...
#ifdef PATHINGMAP
  uniform sampler2D m_PathingMap;
#endif
...
vec2 computeLighting(in vec3 wvPos, in vec3 wvNorm, in vec3 wvViewDir, in vec3 wvLightDir){
  ...
  vec4 calculateDiffuseBlend(in vec2 texCoord) {
    ...
    #ifdef FOGMAP
        vec4 color = texture2D(m_FogMap, texCoord.xy);
        diffuseColor = mix( diffuseColor, color, color.a );
    #endif

    #ifdef PATHINGMAP

        vec4 color = texture2D(m_PathingMap, texCoord.xy);
         diffuseColor = mix( diffuseColor, color, color.a );
    #endif
    ...
    /*
    //Test Grid
    if(int(texCoord.x*1024*3)%2 == 0)
    {
        if(int(texCoord.x*128)%2 == 0 && int(texCoord.x*256)%2 == 0 && int(texCoord.x*(256+128))%2 == 0 && int(texCoord.x*(512))%2 == 0 && int(texCoord.x*(512+256))%2 == 0 && int(texCoord.x*(1024+512))%2 == 0)
        {
            diffuseColor = vec4(1,0,0,1);
        }
    }
    if( int(texCoord.y*1024*3)%2 == 0)
    {

        if(int(texCoord.y*128)%2 == 0 && int(texCoord.y*256)%2 == 0 && int(texCoord.y*(256+128))%2 == 0 && int(texCoord.y*(512))%2 == 0 && int(texCoord.y*(512+256))%2 == 0 && int(texCoord.y*(1024+512))%2 == 0)
        {
            diffuseColor = vec4(1,0,0,1);
        }
    }
...
void main(){
    ...
    //--------------------------
    vec4 oldColor =  AmbientSum * diffuseColor +
                    DiffuseSum * diffuseColor  * light.x +
                    SpecularSum * specularColor * light.y;

    #ifdef GRIDSIZE
        vec4 color = vec4(1,1,1,1);
        float gridValue = clamp(step(dist*0.0005,fract(texCoord.x * float(m_GridSize))) *  step(dist*0.0005,fract(texCoord.y * float(m_GridSize))),0.4,1.0);

        if(m_GridSize == 0 || gridValue == 1)
        {
            gl_FragColor=oldColor;
        }else{
             gl_FragColor=m_GridColor;
        }
    #else
        gl_FragColor=oldColor;
    #endif
  //  gl_FragColor = color * vec4(gridValue) + oldColor;
